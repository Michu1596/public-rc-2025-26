...
class PID:
    ...
    def output_signal(self, commanded_variable: float, sensor_readings: list[float]) -> float:
        ...
        self.integral += 0.5 * (commanded_variable - sensor_readings[0] + commanded_variable - sensor_readings[1]) * self.sensor_period
        # clip integral to avoid windup
        max_val, min_val = abs(self.output_limits[0] / (self.gain_int + 1e-3)), - abs(self.output_limits[1] / (self.gain_int + 1e-3))
        ...
        return clip(proportional + integral - derivative, self.output_limits[0], self.output_limits[1])
    # END OF TODO
